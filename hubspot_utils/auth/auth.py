...
class Auth:
    def __init__(self, app_id, secret_store_name, secrets=None):
        self.log = Logger.get_logger()
        self.hubspot = HubSpot()
        self.config = AppConfig(secret_store_name=secret_store_name, secrets=secrets)
        self.app_id = app_id
    ...
    def generate_and_save_tokens(self, initial=False):
        ...
        self.config.update_secret(f"hubspot_access_token", tokens.access_token)
        if initial:
            # NOTE: If being used for app installation for the first time
            self.config.update_secret(
                f"{self.app_id}_refresh_token", tokens.refresh_token
            )
